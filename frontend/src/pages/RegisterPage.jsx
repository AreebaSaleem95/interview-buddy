...
const benefits = [
  ...
  { icon: '📈', text: 'Category performance tracking and trend charts' },
  { icon: '🏆', text: 'Difficulty levels from beginner to staff engineer' },
];
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-12 bg-background">
      ...
      <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-12 relative overflow-hidden border-r border-border">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-16">
            ...
            <p className="mt-4 text-text-secondary leading-relaxed max-w-xs">
              Practice scenario-based interviews, receive granular feedback, and identify growth areas.
            </p>
          </div>
        </div>
        ...
        <div className="relative z-10 space-y-5">
          {benefits.map((b, i) => (
            ...
            >
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-base border border-primary/20 bg-primary/8">
                {b.icon}
              </div>
              <span className="text-sm text-text-secondary font-medium">{b.text}</span>
            </motion.div>
          ))}
        </div>
      ...
      <div className="lg:col-span-7 flex flex-col items-center justify-center px-6 py-16 lg:px-16">
        <div className="w-full max-w-[400px] space-y-8">
          ...
          <div>
            <h1 className="font-display text-3xl font-extrabold text-text-primary tracking-tight">Create your account</h1>
            <p className="mt-2 text-sm text-text-secondary">Start practicing in under a minute.</p>
          </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {[{ label: 'Google', icon: 'G' }, { label: 'GitHub', icon: '⌥' }].map((p) => (
                <button
                  key={p.label}
                  type="button"
                  onClick={() => toast(`${p.label} sign-up coming soon`)}
